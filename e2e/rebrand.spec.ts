...
const goto = (page: Page, path: string) => page.goto(path, { waitUntil: 'domcontentloaded' });
...
test.describe('wintima rebrand @rebrand', () => {
  ...
  test('home page replaces fabricated sections with issue-approved content', async ({ page }) => {
    ...
    await expect(
      main.getByRole('heading', { name: 'Education in the Heart of Ghana' })
    ).toBeVisible();
    await expect(main.getByRole('heading', { name: '2015-2025 Milestones' })).toBeVisible();
    await expect(
      main.getByRole('heading', { name: 'Every Contribution Keeps a Child in School' })
    ).toBeVisible();
    await expect(main.getByRole('heading', { name: 'Connect With Wintima' })).toBeVisible();

    await expect(main.getByRole('link', { name: /Support Us/i })).toHaveAttribute(
      'href',
      '/donate'
    );
  ...
  test('about page keeps founder content while home page avoids founder teaser', async ({
    page,
  }) => {
    await goto(page, '/');
